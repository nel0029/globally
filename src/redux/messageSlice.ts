...
const messageSlice = createSlice({
    ...
    reducers: {
        ...
        updateConvo: (state, action) => {
            ...
            if (state.conversationList !== null) {
                const convoIndex = state.conversationList.findIndex((convo: ConversationListProps) => convo._id === updatedConvo._id)

                if (convoIndex !== -1) {
                    state.conversationList[convoIndex] = updatedConvo

                    state.conversationList = state.conversationList.sort((a: any, b: any) => a.lastMessageTimestamps - b.lastMessageTimestamps)
                }
            }
        },
        ...
        addNewNotifcation: (state, action) => {
            ...
            return {
                ...state,
                notificationList: state.notificationList ? [...state.notificationList, notification] : [notification],
        ...
        resetNotificationsCount: (state) => {
            if (state.unseenNotification) {
                state.unseenNotification = { ...state.unseenNotification, unseenNotificationsCount: 0 }
            }
