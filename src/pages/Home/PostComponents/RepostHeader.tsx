...
const Rerepost: React.FC<HeaderProps> = ({ repost, authorized }) => {
  ...
  const [initialRepostCaption, setInitialRepostCaption] = useState(
    repost.caption
  );
          ...
          {authorized && (
            <MenuItem>
              ...
              >
                ...
                <p className="whitespace-nowrap">Delete Repost</p>
              </button>
            </MenuItem>
          )}
        </MenuContainer>
        ...
        <Modal setModal={setEditModal}>
          <div className="w-full flex flex-row justify-start items-center gap-x-2 py-1">
            <img
              ...
              src={repost.postAuthorAvatarURL.url}
            />
            <div className="flex-grow flex flex-col justify-center leading-none">
              <p className="text-base font-bold">
                {repost.postAuthorFirstName} {repost.postAuthorMiddleName}{" "}
                {repost.postAuthorLastName}
              </p>
              <p className="text-sm">@{repost.postAuthorUserName}</p>
            </div>
          </div>
      ...
      {deleteModal && (
        <Modal setModal={setDeleteModal}>
          ...
          <div className="w-full flex justify-end items-center gap-x-2">
            ...
            <DeleteButton onClick={[removeRepost]}>Delete</DeleteButton>
          </div>
        </Modal>
      )}
    </div>
