...
const MessageRequestListContainer = () => {
  ...
  const conversationList: any[] = useSelector(
    (state: any) => state.messages.conversationRequestList
  );
  ...
  const openConvo = (conversationID: string) =>
    navigate(`/messages/r/requests/${conversationID}`);
  ...
  return (
    <div className="w-full sticky top-0 h-screen overflow-auto flex-grow flex flex-col gap-y-2 lg:border-r lg:dark:border-Dark300">
      <Header>
        ...
        <TitleText>Message Requests</TitleText>
      </Header>
        ...
        {conversationList && conversationList.length > 0 ? (
          conversationList?.map((conversation: ConversationRequestsProps) => (
            <ConversationRequestCard
              ...
              middleName={
                conversation.requesterMiddleName
                  ? conversation.requesterMiddleName
                  : ""
              }
              lastName={
                conversation.requesterLastName
                  ? conversation.requesterLastName
                  : ""
              }
