...
const CardDetailsHeaderMenu: React.FC<CardHeaderProps> = ({
  updateAction,
  deleteAction,
  ...card
}) => {
          ...
          {authorized && (
            <MenuItem>
              <MenuButton onClick={[openEditModal]}>
                ...
                <p className="whitespace-nowrap">Edit Post Caption</p>
              </MenuButton>
            </MenuItem>
          )}
        ...
        <Modal setModal={setEditModal}>
          <div className="w-full flex flex-row justify-start items-center gap-x-2 py-1">
            <img
              ...
              src={card.postAuthorAvatarURL.url}
            />
            <div className="flex-grow flex flex-col justify-center leading-none">
              <p className="text-base font-bold">
                {card.postAuthorFirstName} {card.postAuthorMiddleName}{" "}
                {card.postAuthorLastName}
              </p>
              <p className="text-sm">@{card.postAuthorUserName}</p>
            </div>
          </div>
